#include "nullspace_admittance_controller.h"
...
{
    bool NullspaceAdmittanceController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& node_handle)
    {
        ...
        dynamic_reconfigure_nullspace_admittance_controller_param_node_ =
        ros::NodeHandle("dynamic_reconfigure_nullspace_admittance_controller_param_node");
        dynamic_server_nullspace_admittance_controller_param_ = std::make_unique<dynamic_reconfigure::Server<nullspace_admittance_controller::nullspace_admittance_controller_paramConfig>>( dynamic_reconfigure_nullspace_admittance_controller_param_node_);
        dynamic_server_nullspace_admittance_controller_param_->setCallback(boost::bind(&NullspaceAdmittanceController::trialControllerVelocityParamCallback, this, _1, _2));
        
        return true;
    }
    
    void NullspaceAdmittanceController::update(const ros::Time&, const ros::Duration& period)
    {
...
//         }
    ...
    void NullspaceAdmittanceController::stopping(const ros::Time&)
    {
    ...
    Eigen::Matrix<double,2,1> NullspaceAdmittanceController::setSafeVelocities(double max_position, double min_position, double max_velocity,double max_acceleration,double max_jerk,double last_commanded_position,double last_commanded_velocity,double last_commanded_acceleration) {
    ...
    void NullspaceAdmittanceController::trialControllerVelocityParamCallback(
    nullspace_admittance_controller::nullspace_admittance_controller_paramConfig& config,
    uint32_t /*level*/) {
        if (!dyn_params_set) {
            config.m_tr = m_tr_;
            config.m_rot = m_rot_;
...
PLUGINLIB_EXPORT_CLASS(nullspace_admittance_controller::NullspaceAdmittanceController,controller_interface::ControllerBase)

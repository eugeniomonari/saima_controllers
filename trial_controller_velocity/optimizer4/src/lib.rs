...
//
// Generated at: 2024-07-05 12:36:52.518890
//
...
pub const OPTIMIZER4_NUM_DECISION_VARIABLES: usize = 8;

/// Number of parameters
pub const OPTIMIZER4_NUM_PARAMETERS: usize = 83;

/// Number of parameters associated with augmented Lagrangian
pub const OPTIMIZER4_N1: usize = 0;

/// Number of penalty constraints
pub const OPTIMIZER4_N2: usize = 14;
...
pub struct optimizer4Cache {
    cache: AlmCache,
}

impl optimizer4Cache {
    pub fn new(cache: AlmCache) -> Self {
        optimizer4Cache { cache }
    }
}
...
///
#[repr(C)]
pub struct optimizer4SolverStatus {
    /// Exit status
    exit_status: optimizer4ExitStatus,
...
#[no_mangle]
pub extern "C" fn optimizer4_new() -> *mut optimizer4Cache {
    Box::into_raw(Box::new(optimizer4Cache::new(initialize_solver())))
}
...
#[no_mangle]
pub unsafe extern "C" fn optimizer4_solve(
    instance: *mut optimizer4Cache,
    u: *mut c_double,
    ...
    let instance: &mut optimizer4Cache = {
    ...
    let u : &mut [f64] = {
        assert!(!u.is_null());
        std::slice::from_raw_parts_mut(u as *mut f64, OPTIMIZER4_NUM_DECISION_VARIABLES)
    };

    // "*const c_double" to "&[f64]"
    let params : &[f64] = {
        assert!(!params.is_null());
        std::slice::from_raw_parts(params as *const f64, OPTIMIZER4_NUM_PARAMETERS)
    };
    ...
    let y0_option: Option<Vec<f64>> = if y0.is_null() {
        ...
        Some(std::slice::from_raw_parts(y0 as *mut f64, OPTIMIZER4_N1).to_vec())
    };

    // Invoke `solve`
    let status = solve(params,&mut instance.cache, u, &y0_option, &c0_option);

    // Check solution status and cast it as `optimizer4SolverStatus`
    match status {
        Ok(status) => optimizer4SolverStatus {
            exit_status: match status.exit_status() {
                core::ExitStatus::Converged => optimizer4ExitStatus::optimizer4Converged,
                core::ExitStatus::NotConvergedIterations => optimizer4ExitStatus::optimizer4NotConvergedIterations,
                core::ExitStatus::NotConvergedOutOfTime => optimizer4ExitStatus::optimizer4NotConvergedOutOfTime,
            },
        ...
        Err(e) => optimizer4SolverStatus {
            exit_status: match e {
                SolverError::Cost => optimizer4ExitStatus::optimizer4NotConvergedCost,
                SolverError::NotFiniteComputation => optimizer4ExitStatus::optimizer4NotConvergedNotFiniteComputation,
            },
...
/// All arguments must have been properly initialised
#[no_mangle]
pub unsafe extern "C" fn optimizer4_free(instance: *mut optimizer4Cache) {
    // Add impl
    assert!(!instance.is_null());
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(OPTIMIZER4_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, OPTIMIZER4_N1, OPTIMIZER4_N2)
}
...
pub fn solve(
    ...
    assert_eq!(p.len(), OPTIMIZER4_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), OPTIMIZER4_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_optimizer4::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_optimizer4::grad(u, xi, p, grad);
        Ok(())
    };
    
    let f2 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_optimizer4::mapping_f2(u, p, res);
        Ok(())

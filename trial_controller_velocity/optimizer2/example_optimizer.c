 ...
 * gcc -Wall -std=c99 -pedantic \
    example_optimizer.c -l:liboptimizer2.a \
...
int main() {
    ...
    /* parameters             */
    double p[OPTIMIZER2_NUM_PARAMETERS] = {2.0, 10.0};

    /* initial guess          */
    double u[OPTIMIZER2_NUM_DECISION_VARIABLES] = {0};
    ...
    /* solve                  */
    optimizer2SolverStatus status = optimizer2_solve(cache, u, p, 0, &init_penalty);
    ...
    for (i = 0; i < OPTIMIZER2_NUM_DECISION_VARIABLES; ++i) {
        printf("u[%d] = %g\n", i, u[i]);
    }

    printf("\n");
    for (i = 0; i < OPTIMIZER2_N1; ++i) {
        printf("y[%d] = %g\n", i, status.lagrange[i]);
    }

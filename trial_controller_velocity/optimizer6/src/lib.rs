...
//
// Generated at: 2024-07-16 11:05:22.006296
//
...
pub const OPTIMIZER6_NUM_DECISION_VARIABLES: usize = 6;

/// Number of parameters
pub const OPTIMIZER6_NUM_PARAMETERS: usize = 53;

/// Number of parameters associated with augmented Lagrangian
pub const OPTIMIZER6_N1: usize = 0;

/// Number of penalty constraints
pub const OPTIMIZER6_N2: usize = 16;
...
pub struct optimizer6Cache {
    cache: AlmCache,
}

impl optimizer6Cache {
    pub fn new(cache: AlmCache) -> Self {
        optimizer6Cache { cache }
    }
}
...
///
#[repr(C)]
pub struct optimizer6SolverStatus {
    /// Exit status
    exit_status: optimizer6ExitStatus,
...
#[no_mangle]
pub extern "C" fn optimizer6_new() -> *mut optimizer6Cache {
    Box::into_raw(Box::new(optimizer6Cache::new(initialize_solver())))
}
...
#[no_mangle]
pub unsafe extern "C" fn optimizer6_solve(
    instance: *mut optimizer6Cache,
    u: *mut c_double,
    ...
    let instance: &mut optimizer6Cache = {
    ...
    let u : &mut [f64] = {
        assert!(!u.is_null());
        std::slice::from_raw_parts_mut(u as *mut f64, OPTIMIZER6_NUM_DECISION_VARIABLES)
    };

    // "*const c_double" to "&[f64]"
    let params : &[f64] = {
        assert!(!params.is_null());
        std::slice::from_raw_parts(params as *const f64, OPTIMIZER6_NUM_PARAMETERS)
    };
    ...
    let y0_option: Option<Vec<f64>> = if y0.is_null() {
        ...
        Some(std::slice::from_raw_parts(y0 as *mut f64, OPTIMIZER6_N1).to_vec())
    };

    // Invoke `solve`
    let status = solve(params,&mut instance.cache, u, &y0_option, &c0_option);

    // Check solution status and cast it as `optimizer6SolverStatus`
    match status {
        Ok(status) => optimizer6SolverStatus {
            exit_status: match status.exit_status() {
                core::ExitStatus::Converged => optimizer6ExitStatus::optimizer6Converged,
                core::ExitStatus::NotConvergedIterations => optimizer6ExitStatus::optimizer6NotConvergedIterations,
                core::ExitStatus::NotConvergedOutOfTime => optimizer6ExitStatus::optimizer6NotConvergedOutOfTime,
            },
        ...
        Err(e) => optimizer6SolverStatus {
            exit_status: match e {
                SolverError::Cost => optimizer6ExitStatus::optimizer6NotConvergedCost,
                SolverError::NotFiniteComputation => optimizer6ExitStatus::optimizer6NotConvergedNotFiniteComputation,
            },
...
/// All arguments must have been properly initialised
#[no_mangle]
pub unsafe extern "C" fn optimizer6_free(instance: *mut optimizer6Cache) {
    // Add impl
    assert!(!instance.is_null());
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(OPTIMIZER6_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, OPTIMIZER6_N1, OPTIMIZER6_N2)
}
...
pub fn solve(
    ...
    assert_eq!(p.len(), OPTIMIZER6_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), OPTIMIZER6_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_optimizer6::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_optimizer6::grad(u, xi, p, grad);
        Ok(())
    };
    
    let f2 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_optimizer6::mapping_f2(u, p, res);
        Ok(())

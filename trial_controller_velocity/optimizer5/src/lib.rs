...
//
// Generated at: 2024-07-05 13:17:56.347616
//
...
pub const OPTIMIZER5_NUM_DECISION_VARIABLES: usize = 2;

/// Number of parameters
pub const OPTIMIZER5_NUM_PARAMETERS: usize = 83;

/// Number of parameters associated with augmented Lagrangian
pub const OPTIMIZER5_N1: usize = 0;

/// Number of penalty constraints
pub const OPTIMIZER5_N2: usize = 14;
...
pub struct optimizer5Cache {
    cache: AlmCache,
}

impl optimizer5Cache {
    pub fn new(cache: AlmCache) -> Self {
        optimizer5Cache { cache }
    }
}
...
///
#[repr(C)]
pub struct optimizer5SolverStatus {
    /// Exit status
    exit_status: optimizer5ExitStatus,
...
#[no_mangle]
pub extern "C" fn optimizer5_new() -> *mut optimizer5Cache {
    Box::into_raw(Box::new(optimizer5Cache::new(initialize_solver())))
}
...
#[no_mangle]
pub unsafe extern "C" fn optimizer5_solve(
    instance: *mut optimizer5Cache,
    u: *mut c_double,
    params: *const c_double,
    y0: *const c_double,
    c0: *const c_double,
) -> optimizer5SolverStatus {

    // Convert all pointers into the required data structures
    let instance: &mut optimizer5Cache = {
    ...
    let u : &mut [f64] = {
        assert!(!u.is_null());
        std::slice::from_raw_parts_mut(u as *mut f64, OPTIMIZER5_NUM_DECISION_VARIABLES)
    };

    // "*const c_double" to "&[f64]"
    let params : &[f64] = {
        assert!(!params.is_null());
        std::slice::from_raw_parts(params as *const f64, OPTIMIZER5_NUM_PARAMETERS)
    };
    ...
    let y0_option: Option<Vec<f64>> = if y0.is_null() {
        ...
        Some(std::slice::from_raw_parts(y0 as *mut f64, OPTIMIZER5_N1).to_vec())
    };

    // Invoke `solve`
    let status = solve(params,&mut instance.cache, u, &y0_option, &c0_option);

    // Check solution status and cast it as `optimizer5SolverStatus`
    match status {
        Ok(status) => optimizer5SolverStatus {
            exit_status: match status.exit_status() {
                core::ExitStatus::Converged => optimizer5ExitStatus::optimizer5Converged,
                core::ExitStatus::NotConvergedIterations => optimizer5ExitStatus::optimizer5NotConvergedIterations,
                core::ExitStatus::NotConvergedOutOfTime => optimizer5ExitStatus::optimizer5NotConvergedOutOfTime,
            },
        ...
        Err(e) => optimizer5SolverStatus {
            exit_status: match e {
                SolverError::Cost => optimizer5ExitStatus::optimizer5NotConvergedCost,
                SolverError::NotFiniteComputation => optimizer5ExitStatus::optimizer5NotConvergedNotFiniteComputation,
            },
...
/// All arguments must have been properly initialised
#[no_mangle]
pub unsafe extern "C" fn optimizer5_free(instance: *mut optimizer5Cache) {
    // Add impl
    assert!(!instance.is_null());
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(OPTIMIZER5_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, OPTIMIZER5_N1, OPTIMIZER5_N2)
}
...
pub fn solve(
    ...
    assert_eq!(p.len(), OPTIMIZER5_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), OPTIMIZER5_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_optimizer5::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_optimizer5::grad(u, xi, p, grad);
        Ok(())
    };
    
    let f2 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_optimizer5::mapping_f2(u, p, res);
        Ok(())
